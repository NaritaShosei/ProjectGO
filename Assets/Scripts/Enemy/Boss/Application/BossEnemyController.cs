...
namespace BossEnemy.Application
{
    ...
    public class BossEnemyController
    {
        ...
        private void RegisterArmorDamageEventAction()
        {
            ...
            {
                ArmorHPFluctuationEventAction(hp, _currentPhaseBossEnemyData.Value.LeftArmArmer, ArmorAttachmentPointType.LeftArm);
            }).AddTo(_phaseChangeEventDisposables);

            // 右腕の鎧のHP変動時
            _currentPhaseBossEnemyData.Value.RightArmArmer.CurrentHP.Subscribe(hp =>
            {
                ArmorHPFluctuationEventAction(hp, _currentPhaseBossEnemyData.Value.RightArmArmer, ArmorAttachmentPointType.RightArm);
            }).AddTo(_phaseChangeEventDisposables);

            // 左足の鎧のHP変動時
            _currentPhaseBossEnemyData.Value.LeftLegArmer.CurrentHP.Subscribe(hp =>
            {
                ArmorHPFluctuationEventAction(hp, _currentPhaseBossEnemyData.Value.LeftLegArmer, ArmorAttachmentPointType.LeftLeg);
            }).AddTo(_phaseChangeEventDisposables);

            // 右足の鎧のHP変動時
            _currentPhaseBossEnemyData.Value.RightLegArmer.CurrentHP.Subscribe(hp =>
            {
                ArmorHPFluctuationEventAction(hp, _currentPhaseBossEnemyData.Value.RightLegArmer, ArmorAttachmentPointType.RightLeg);
            }).AddTo(_phaseChangeEventDisposables);
        }

        private void ArmorHPFluctuationEventAction(int currentHP, BossArmorData armorData, ArmorAttachmentPointType attachmentPointsType)
        {
            if (currentHP == 0)
            {
