...
通常カメラがFollowするプレイヤー追従アンカー（内部生成の `CameraFollowTarget`）は `FollowAnchor` プロパティで公開しており、`BossCameraController` がボスカメラのFollowにそのまま流用します。ボスカメラ有効中は `CameraMotionController` のTickが止まるため、アンカーの位置更新は `BossCameraController` 側が肩代わりします。
...
- 注視（体をむく）：ボスの子から `CameraAnglePoint`（`Top`＝頭側 / `Under`＝足元側 / `Left`・`Right`＝左右）を集め、プレイヤー↔ボス距離を `_bossFramingNearDistance`〜`_bossFramingFarDistance` で 0..1 に正規化し、その比率で注視プロキシを 足元基準点→`Top` で線形補間（`Near`以下＝足元、`Far`以上＝頭）。足元基準点は `Left`＋`Right` が両方揃っていればその中点（`TryGetRawUnderPosition`）を優先し、無ければ `Under` 単体を使う。左右どちらか一方の脚に基準点が寄っていると、左右スイベルでその脚だけを横切るときにロックオンの対象切替のように見えてしまうため、中点を使って軸をボスの正中線に合わせている。アンカーが無ければ `IEnemy.GetTargetCenter()` へフォールバック
- 足元基準点の生位置（`TryGetRawUnderPosition`）には `TryGetUnderPosition` で `_bossUnderSmoothTime`（秒）の `SmoothDamp` を1段掛ける（`Under`/`Left`/`Right` が歩行アニメ等のボーンに付いている場合の揺れを抑えるため）。ボスの並行移動・旋回にはそのまま追従する。有効化時（`CollectAnglePoints`）にスムージングの起点を現在位置へスナップするため、出現直後にゼロから寄っていく事故は起きない
